...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'name') {
            ...
            $_data .= '<a target="_blank" href="' . base_url('zillapage/publish/' . $aRow['code']) . '">' . _l('preview') . '</a>';
    ...
    $_data .= '<a href="'.base_url('zillapage/publish/'. $aRow['code']).'" class="btn btn-sm btn-default" target="_blank">'._l('preview').'</a>';
