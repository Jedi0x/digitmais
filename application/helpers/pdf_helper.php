...
function pdf_logo_url_updated($width,$style='')
{
    ...
    if ($logoUrl != '') {
        ...
        $logoImage = '<img width="' . $width . 'px" src="' . $logoUrl . '" '.$style.' >';
    }
 ...
 * @param  object $task task as object with all necessary fields
 * @param  string $tag     tag for bulk pdf exporter
 * @return mixed object
 */
function task_pdf($task, $tag = '')
{
    return app_pdf('task', LIBSPATH . 'pdf/Task_pdf', $task, $tag);
}
