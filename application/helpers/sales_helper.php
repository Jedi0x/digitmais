...
function format_customer_info_updated($data, $for, $type, $companyLink = false)
{
    ...
    $clientId = $data->project_data->client_data->userid;
    $companyName = $data->project_data->client_data->company;
    ...
    if (in_array($for, $acceptsPrimaryContactDisplay) &&
        isset($data->project_data->client_data->show_primary_contact) &&
        $data->project_data->client_data->show_primary_contact == 1 &&
        $primaryContactId = get_primary_contact_user_id($clientId)) {
        $companyName = get_contact_full_name($primaryContactId) . '<br />' . $companyName;
    }
...
$street  = !empty($data->project_data->client_data->address) ? $data->project_data->client_data->address : '';
$city    = !empty($data->project_data->client_data->city) ? $data->project_data->client_data->city : '';
$state   = !empty($data->project_data->client_data->state) ? $data->project_data->client_data->state : '';
$zipCode = !empty($data->project_data->client_data->zip) ? $data->project_data->client_data->{$type . '_zip'} : '';

$countryCode = '';
$countryName = '';

if ($country = !empty($data->project_data->client_data->country) ? get_country($data->project_data->client_data->country) : '') {
    $countryCode = $country->iso2;
    $countryName = $country->short_name;
}

$phone = '';
if ($type == 'billing' && isset($data->project_data->client_data->phonenumber)) {
    $phone = $data->project_data->client_data->phonenumber;
}

$vat = '';
if ($type == 'billing' && isset($data->project_data->client_data->vat)) {
    $vat = $data->project_data->client_data->vat;
}
